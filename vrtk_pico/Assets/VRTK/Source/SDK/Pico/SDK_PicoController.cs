...
namespace VRTK
{
    ...
    /// <summary>
    /// The Pico Controller SDK script provides a bridge to SDK methods that deal with the input devices.
    /// </summary>
    [SDK_Description(typeof(SDK_PicoSystem))]

    public class SDK_PicoController
#if VRTK_DEFINE_SDK_PICO
        : SDK_BaseController
#else
...
#endif
    {
        ...
        //protected Vector2[] thumbsticks = new Vector2[] { Controller.UPvr_GetTouchPadPosition(0), Controller.UPvr_GetTouchPadPosition(1) };
        //protected Controller.[] touchpads = new bool[] { Controller.UPvr_GetKey(0, Pvr_KeyCode.TRIGGER) ? true : false, Controller.UPvr_GetKey(1, Pvr_KeyCode.TRIGGER) ? true : false };
        //protected bool[] triggers = new bool[] { Controller.UPvr_GetKey(0, Pvr_KeyCode.TRIGGER) ? true : false, Controller.UPvr_GetKey(1, Pvr_KeyCode.TRIGGER) ? true : false };
        ...
        private Vector2 tempAxis = Vector2.zero;

        private bool touchpadIsDown = false;
        ...
        {
            if (GetCurrentControllerType() == ControllerType.Custom)
            {
                return "ControllerColliders/Neo2_" + hand.ToString();
            }
        ...
        public override GameObject GetControllerLeftHand(bool actual = false)
        {
            ...
            {
                controller = VRTK_SharedMethods.FindEvenInactiveGameObject<SDK_PicoCameraRig>("TrackingSpace/LeftHandAnchor", true);
            }
            return controller;
        }
        ...
        public override GameObject GetControllerRightHand(bool actual = false)
        {
            ...
            if (controller == null && actual)
            {
                controller = VRTK_SharedMethods.FindEvenInactiveGameObject<SDK_PicoCameraRig>("TrackingSpace/RightHandAnchor", true);
            }
            return controller;
        }
        ...
        {
            //If Pico avatar isn't being used or the default model isn't set or the current controller model isn't the default controller model, then don't bother waiting for the model to stream in.
            if (HasAvatar())
            {
        ...
        public override void HapticPulse(VRTK_ControllerReference controllerReference, float strength = 0.5f)
        {
            if (VRTK_ControllerReference.IsValid(controllerReference))
            {
                ...
                {
                    Controller.UPvr_VibrateNeo2Controller((byte)strength * byte.MaxValue, 1, 0);
                }
                else if (IsControllerRightHand(controller))
                {
                    Controller.UPvr_VibrateNeo2Controller((byte)strength * byte.MaxValue, 1, 1);
                }
            }
        }
        ...
        public override Vector2 GetButtonAxis(ButtonTypes buttonType, VRTK_ControllerReference controllerReference)
        {
            ...
            switch (buttonType)
            {
                case ButtonTypes.Touchpad:
                    return GetThumbstickValue(controllerReference);
                case ButtonTypes.Trigger:
                    return new Vector2(Controller.UPvr_GetControllerTriggerValue((int)controllerReference.index) / 255, 0); ;
            }
        ...
        public override bool GetControllerButtonState(ButtonTypes buttonType, ButtonPressTypes pressType, VRTK_ControllerReference controllerReference)
        {
            ...
            int index = (int)VRTK_ControllerReference.GetRealIndex(controllerReference);
            ...
            switch (buttonType)
            {
                case ButtonTypes.Trigger:
                    switch (pressType)
                    {
                        ...
                        case ButtonPressTypes.PressUp:
                            return IsButtonPressed(index, pressType, Pvr_KeyCode.TRIGGER);
                ...
                case ButtonTypes.Grip:
                    switch (pressType)
                    {
                        ...
                        case ButtonPressTypes.PressUp:
                            if (index == 0)
                            {
                                return IsButtonPressed(index, pressType, Pvr_KeyCode.Left);
                            }
                            return IsButtonPressed(index, pressType, Pvr_KeyCode.Right);
                        case ButtonPressTypes.Touch:
                        case ButtonPressTypes.TouchDown:
                        case ButtonPressTypes.TouchUp:
                            if (index == 0)
                ...
                case ButtonTypes.ButtonOne:
                    switch (pressType)
                    {
                        ...
                        case ButtonPressTypes.PressUp:
                            if (index == 0)
                            {
                                return IsButtonPressed(index, pressType, Pvr_KeyCode.X);
                            }
                            return IsButtonPressed(index, pressType, Pvr_KeyCode.A);
                ...
                case ButtonTypes.ButtonTwo:
                    switch (pressType)
                    {
                        ...
                        case ButtonPressTypes.PressUp:
                            if (index == 0)
                            {
                                return IsButtonPressed(index, pressType, Pvr_KeyCode.Y);
                            }
                            return IsButtonPressed(index, pressType, Pvr_KeyCode.B);
                        case ButtonPressTypes.Touch:
                        case ButtonPressTypes.TouchDown:
                        case ButtonPressTypes.TouchUp:
                            if (index == 0)
                ...
                case ButtonTypes.Touchpad:
                    switch (pressType)
                    {
                        ...
                        case ButtonPressTypes.PressDown:
                        case ButtonPressTypes.PressUp:
                            return IsButtonPressed(index, pressType, Pvr_KeyCode.TOUCHPAD);
                        case ButtonPressTypes.TouchDown:
                            return GetTouchpadTouchState(index, ButtonPressTypes.TouchDown);
                        case ButtonPressTypes.Touch:
                            return GetTouchpadTouchState(index, ButtonPressTypes.Touch);
                        case ButtonPressTypes.TouchUp:
                            return GetTouchpadTouchState(index, ButtonPressTypes.TouchUp);
                    }
                    break;
                case ButtonTypes.StartMenu:
                    switch (pressType)
                    {
                        ...
                        case ButtonPressTypes.PressDown:
                        case ButtonPressTypes.PressUp:
                            return IsButtonPressed(index, pressType, Pvr_KeyCode.APP);
                        case ButtonPressTypes.Touch:
                        case ButtonPressTypes.TouchDown:
                        case ButtonPressTypes.TouchUp:
                            return IsButtonPressed(index, pressType, Pvr_KeyCode.APP);
        ...
        {
            if (CheckAxisTouch(index))
            {
                if (type == ButtonPressTypes.TouchDown)
                {
                    if (StartTouchpadTouch())
                    {
                        return true;
                    }
                    else
                ...
                }else if(type == ButtonPressTypes.Touch)
                {
                    return touchpadIsDown;
                }
            }
            else
            {
                return EndTouchpadTouch();
            }
        ...
        {
            return (Pvr_KeyCode)Enum.Parse(typeof(Pvr_KeyCode), "Joystick" + index + "Button" + code);
        }
        ...
        protected virtual bool CheckAxisTouch(int index)
        {
            ...
            bool isTouching = !(GetThumbstickValue(index) == Vector2.zero);
            //Debug.Log($"{GetThumbstickValue(index)} for {isTouching}");
            return isTouching;
        }
        ...
        protected virtual bool IsButtonPressed(int index, ButtonPressTypes type, Pvr_KeyCode button)
        {
            VRTK_TrackedController device = GetTrackedObject(GetControllerByIndex((uint)index));
            if (device != null)
            {

                if (button == Pvr_KeyCode.TOUCHPAD)
                {
                    switch (type)
                    {
                        case ButtonPressTypes.Touch:
                        case ButtonPressTypes.TouchDown:
                            return CheckAxisTouch(index);
                        case ButtonPressTypes.TouchUp:
                            return !CheckAxisTouch(index);
                        case ButtonPressTypes.Press:
                            return Controller.UPvr_GetKey(index, button);
                        case ButtonPressTypes.PressDown:
                            return Controller.UPvr_GetKeyDown(index, button);
                        case ButtonPressTypes.PressUp:
                            return Controller.UPvr_GetKeyUp(index, button);
                    }
                }
                else
                {
        ...
        protected virtual SDK_PicoBoundaries GetBoundariesSDK()
        {
            ...
            {
                cachedBoundariesSDK = (VRTK_SDKManager.instance ? VRTK_SDKManager.instance.loadedSetup.boundariesSDK : CreateInstance<SDK_PicoBoundaries>()) as SDK_PicoBoundaries;
            }
